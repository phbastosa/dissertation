...
A modelagem e a tomografia sísmicas são técnicas essenciais na exploração de recursos naturais subterrâneos e na caracterização da subsuperfície terrestre. A tomografia sísmica é um método não invasivo que permite mapear as propriedades físicas do subsolo através da análise das ondas sísmicas geradas por fontes controladas. Um dos principais desafios na tomografia sísmica é a obtenção de informações precisas sobre as trajetórias das ondas sísmicas e suas velocidades de propagação em diferentes camadas geológicas. Nesse sentido, a equação eikonal  é uma ferramenta fundamental para a modelagem, a inversão e o imageamento sísmico, pois descreve as frentes de onda e permite calcular os tempos de percurso das ondas sísmicas. Neste capítulo é explorada a equação eikonal com três resoluções numéricas e sua aplicação na tomografia de refração. Ao explorar esses tópicos, este capítulo fornecerá conceitos utilizados na geração dos resultados deste trabalho.
...
Considerando a utilização de dados sintéticos, a formulação aplicada neste trabalho é analítica baseada no somatório da energia do dado em janelas de varredura temporal. \citeonline{pan2019automatic} calculam dois parâmetros relacionados com o traço sísmico 
\begin{equation}
...
\begin{equation}
	F = |(B / A)\times(B - A)|,
\end{equation} 
\noindent sendo $F$ a indicação de existência de primeiras chegadas no dado. \citeonline{qin2021first} adicionam novos termos para se obter indicações de primeiras chegadas mais precisas formulando a equação
\begin{equation}
